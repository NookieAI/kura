...
- **Completion Constellations.** A Stats view that draws each numbered series as a constellation — owned entries bright, missing sequels dim — with a per-series completion ring, derived from what you own.
- **Golden Fingerprint & Sector-Surgeon.** A container-agnostic game identity (the same game as ISO and as BIN/CUE matches) and a CD ECC/EDC repair engine for near-miss disc dumps, with honest unrecoverable reporting.
- **PS4 saves in the Save Manager.** PS4 backward-compat (sealed) saves are no longer skipped by the bulk backup.
- **Paint-splash theme switch** and smooth colour crossfade, a WCAG-verified readability pass, fully on-theme modals across every palette, and a batch of dead/broken-control fixes.

## 1.5.0–1.5.3 — Own cover CDN, PlayStation-only filter, Genome Map

- **Cover art from Nookie's own CDN** (`retroforge-cdn.pages.dev`) — a curated index of ~28,000 covers spanning PS1 through PS5, PSP and Vita, replacing the libretro dependency.
- **Whitelist-only scan filter** so only genuine PlayStation content is shown, plus **backport-aware duplicate detection** (PS5 backports and firmware variants are no longer flagged as dupes).
- **Genome Map**, a **Curator** view, and scrollable Stats.

## 1.3.0–1.4.0 — Self-updating loader & Media Gallery

- The console loader **auto-updates itself** when it falls behind the app.
- **Media Gallery** — browse and download console screenshots and video clips.
- Live PS5 **temperature**, a read-only **registry browser**, and a **boot-profile** on the dashboard.
